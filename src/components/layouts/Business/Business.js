...
class Business extends React.Component {
  ...
  componentDidMount = async () => {
          ...
          axios.get(`/api/all/basket/${this.state.businessID}`).then((res) => {
            ...
            try {
              ...
              for (let i = itemArr.length - 1; i >= 0; i--) {
                itemArr[i].item = generalUtil.itemNameConverter(
                  itemArr[i].item
                );

                for (let j = 0; j < itemArr.length; j++) {
                  itemArr[j].item = generalUtil.itemNameConverter(
                    itemArr[j].item
                  );
    ...
    axios
      .get(`/api/basket/${this.state.businessID}/${new Date().getTime()}`)
      .then((res) => {
      ...
      return (
        <div className="Business">
          ...
          <div className="business-table-list-container">
            ...
            <BusinessBasketList
              businessID={this.state.businessID}
              className="business-basket-list-container"
            />
          </div>
        </div>
      );
